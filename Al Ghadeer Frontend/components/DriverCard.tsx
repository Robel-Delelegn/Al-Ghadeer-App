...
const DriverCard = ({ item, selected, setSelected }: DriverCardProps) => {
  const averageRating = item.metrics?.average_rating ?? 0;
  const dailyEarnings = item.earnings?.daily_earnings ?? 0;
      ...
      <Image
        source={
          item.profile_image
            ? {
                uri:
                  resolveResourceUrl(item.profile_image) || item.profile_image,
              }
            : icons.person
        }
